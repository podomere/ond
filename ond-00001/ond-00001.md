...
> It will also take time to reload the latest configurations to your devices thus impacting your ```MTTR``` and ```Availability``` metrics, so ensure you always have current backups of all configurations (after every change) and a method to restore them to each device. Remember that backups are almost worthless unless a valid restore can be performed. Without getting in to a discussion on _Change Management_ it's quite easy (with many devices and vendors) to trigger an automatic off-site backup of an updated config once changed or committed. 
