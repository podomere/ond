...
> It may also take time to reload the latest configurations which will impact your ```MTTR``` and thus your ```Availablity``` metrics so ensure you have current backups of all configurations and a method to restore to all devices. Remember that backups are almost worthless unless a valid restore can be performed.
