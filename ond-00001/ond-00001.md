...
**Note:** All ```48``` gigabit ports of the _listed_ switch supply PoE+ and there are an additional ```4``` ports which are SFP/SFP+ (which support ```1 Gbps``` or ```10 Gbps``` for a range of further options). All access edge devices are cloud managed for consistent policy application and enforcement.
