...
Access points should be placed according to a full site-survey or at least a predictive modelling excercise (based upon floorplans and material characteristics being inputted). If neither of these options are possible for whatever reason, then LOS(Line of Sight) combined with avoiding any obvious electromagnetic interference sources/absorbent materials _should_ help dictate the best location for mounting (combined with floorplans and some common sense!). Do not mount access points close to electrical sources or on airconditioning vents. Consult the vendor documentation and best current practices to avoid serious performance and quality issues during the lifetime of the service.

High density principles must also be used especially for spaces that double as event spaces. Particular attention must be paid to channel reuse, co-channel interference, and general interference. Use as many discrete and **clear channels** as possible in the space to form micro-cells. Albeit the nature of any unlicensed radiospace can change dramatically due to environmental factors, neighbour tenancies, and portable broadcast devices; the RF(Radio Frequency) space should be sampled and surveyed both pre and post install. It is generally also better to use static channels rather than leave them on _auto_ (as not many vendors AUto-RF works well) unless a policy can be implemented within the platform to determine under what conditions the channels automatically change and when. More often than not you can cooperate with your neighbours to work around static channels, but if everyone uses auto then there is almost no predictability and increased disruption.

Fundamentally the RF(Radio Frequency) desirables are a minimum -67 dBm RSSI, minimum 25 dB SNR, and 3-6 metres of overlap at these signal levels between APs.

**NoteI:** In smaller spaces the channel widths (channel bonding) can potentially be increased to take advantage of 40MHz, 80MHz and soon 160MHz however it is always preferable to find clean air and to scale extra access points in to available spectrum by virtue of micro-cells for capacity. For high density deployments 20MHz channel widths are preferred for coverage and channel flexibility however this can increase airtime if heavy utilisation is expected. One has to balance 'clean air' and capacity all the while designing for the best SINR(Signal to Interference Plus Noise) possible.

> Great reference for surveying (and all things WiFi/RF design): [http://www.revolutionwifi.net/2013/08/wi-fi-site-surveying-101.html](http://www.revolutionwifi.net/2013/08/wi-fi-site-surveying-101.html)
...
There should only be a maximum of [3-5](http://www.revolutionwifi.net/2013/10/ssid-overhead-how-many-wi-fi-ssids-are.html) SSID's per access point (and/or radio) depending upon frequency and channel due to overheads from beaconing. Albeit some vendors say that up to 16 or 64 SSIDs can be configured, it would in factt bring the radiospace to a crawl. For optimal operation and association the SSID's should be split by dedicated frequency range (which also aids troubleshooting end points).
...
If possible via the vendors radio settings, disable all speeds and MCS indices below 18 Mbps (dependent upon site characteristics but definitely disable below 12 Mbps unless you are intentionaly running 802.11a/b)
...
**NoteII:** Radio power should generally be kept low per radio (10dBm-14dBm)and work from more powerful to less powerful with regard to newer transmission types i.e. 802.11ac(5GHz) -> 802.11n(5GHz) -> 802.11n/g(2.4GHz) which encourages devices to prefer stronger, faster, and cleaner signals where and when they can see them. This discourages devices to connect to 2.4GHz even with dedicated SSIDs and 'band steering' albeit 2.4GHz signals tend to always look stronger, they are in fact severely polluted in most environments.

**NoteIII:** One of the main issues can be with whats called 'hidden node' regarding stations/devices that can not hear each other but can hear the access point, so trying to ensure that all local clients can hear one another in a 'cell' is very important. Taking in to consideration vertical as well as horizontal floors in a building is also crucial if you are to provide a wireless service across floors. 
...
<input type="hidden" name="hosted_button_id" value="JPYHBPU8TT3YS">
