...
All ```48``` gigabit ports of the listed switch supply PoE+ and there are an additional ```4``` ports which are SFP/SFP+ (which support ```1 Gbps``` or ```10 Gbps``` for a range of further options). All access edge devices are cloud managed for consistent policy application and enforcement.
...
Aerohive access points and switches use 'Classifier Maps' and 'Marker Maps' which allow one to place traffic in to logical classes/containers that the infrastructure then treats separately based upon certain attributes and user policies.
...
* "All VOIP and video conferencing" is exempt from any and all shaping and marked with ```CS7/DSCP56``` 
* RTMP (```UDP/TCP 1935```)
* STUN (```UDP/TCP 3478```)
...
* SSH Default (UDP/TCP 22) is expedited (EF) on the primary link* Windows Remote Desktop (UDP/TCP 3389) is expedited on primary link 
* IAX(Inter-Asterisk eXchange) (UDP/TCP 4569) is expedited on primary link* Yahoo Messenger (UDP/TCP 5101) is expedited on primary link* RTP(Realtime Transport Protocol) (UDP/TCP 5004-5005) are expedited on primary link 
* SIP(Session Initiation Protocol) (UDP/TCP 5060-5062) are expedited on primary link 
* VNC(Virtual Network Computing) (UDP/TCP 5800) is expedited on primary link 
* VNC-Server (UDP/TCP 5900-5901) are expedited on primary link* TeamViewer (UDP/TCP 5938) is expedited on primary link* Cisco WebEx (UDP/TCP 8070, 8090) is expedited on primary link* TeamSpeak (UDP/TCP 8767) is expedited on primary link* RTP (mainly used after SIP) (UDP/TCP 16384-16482) are expedited on primary link 
* Google Hangouts (UDP/TCP 19305-19309) are expedited on primary link
...
* ICMP (echo_request/reply i.e. pings) via the primary link
...
| Device A <br>Port ID | A Presentation | Cable Type | Patch X ID | Patch Y ID | Cable Type | B Presentation | Device B <br>Port ID |
| ------------------------ |-------------| -----| -----| --- | ---- | ---- | ----- |
...
| Device A <br>Port ID | A Presentation | Cable Type | Patch X ID | Patch Y ID | Cable Type | B Presentation | Device B <br>Port ID | Acesss / Trunk |
| ------------------------ |-------------| -----| -----| --- | ---- | ---- | --------------: | ------ |
...
The availability of commercial vendor support 8x5 business hours (with optional upgrade to 24/7) is paramount as is hardware replacement. Additional coverage for next day 
...
**Note:** Of course one can mix and match to create one's own pod.


## Attribution

This design artefact is Company Confidential / Commercial in Confidence for Inspire9 Pty Ltd created [@podomere](http://twitter.com/podomere) however it is built using Public Domain templates in the OND(Open Network Design) project available @ [http://ond.podome.re](http://ond.podome.re) 

-> ~~~~ ```O```pen ```N```etwork ```D```esign ~~~~ <-

Should the client Inspire9 Pty Ltd wish to share this design with others or release it in to the Public Domain it is required that Inspire9 Pty Ltd shares this document with this Atribution notice and include the CC Attribution-ShareAlike 4.0 International (CC BY-SA 4.0) license [http://creativecommons.org/licenses/by-sa/4.0/](http://creativecommons.org/licenses/by-sa/4.0/ ). We would also welcome contributions and feed back to the OND project also of course!